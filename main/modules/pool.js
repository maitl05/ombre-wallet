...
export class Pool {
    ...
    try {
      ...
      const start =
        ...
        JSON.stringify(this.config.server) !=
          JSON.stringify(options.pool.server)

      let update_work = false
      if (!start && this.active) {
        if (
          this.config.mining.address != options.pool.mining.address ||
          this.config.mining.uniform != options.pool.mining.uniform
        ) {
          update_work = true
        }
      }
      ...
      if (!start && this.active) {
        if (
          JSON.stringify(this.config.varDiff) !=
          JSON.stringify(options.pool.varDiff)
        ) {
          update_vardiff = true
        }
      }
    ...
    this.stop().then(() => {
      ...
      this.intervals.startup = setInterval(() => {
        ...
        const target_height = Math.max(
          daemon_info.height_without_bootstrap,
          daemon_info.target_height,
        )

        if (
          daemon_info.is_ready &&
          daemon_info.height_without_bootstrap >= target_height
        ) {
          ...
          if (debounce++ % 30 == 0) {
            logger.log(
              'info',
              'Wait for daemon { is_ready: %s, height: %d, target_height: %d }',
              [
                daemon_info.is_ready,
                daemon_info.height_without_bootstrap,
                target_height,
  ...
  startHeartbeat() {
    ...
    this.intervals.timeout = setInterval(() => {
      for (let connection_id in this.connections) {
        ...
        if (
          Date.now() - miner.lastHeartbeat >
          this.config.mining.minerTimeout * 1000
        ) {
          logger.log('warn', 'Worker timed out %s@%s', [
            miner.workerName,
            miner.ip,
          ])
    ...
    this.checkHeight()
      .then((response) => {
        try {
          ...
          if (
            json.data.hasOwnProperty('height') &&
            ...
            if (desynced) {
              logger.log(
                'error',
                'Pool height is desynced { remote: %d, local: %d }',
                [remote_height, this.blocks.current.height],
              )
            } else {
              logger.log(
                'info',
                'Pool height is okay { remote: %d, local: %d }',
                [remote_height, this.blocks.current.height],
              )
            }
          }
          if (json.data.hasOwnProperty('server_time')) {
            ...
            system_clock_error =
              Math.abs(server_time - system_time) > allowed_time_variance
            if (system_clock_error) {
              logger.log(
                'error',
                'System clock is not correct { server: %d, local: %d }',
                [server_time, system_time],
              )
            } else {
              logger.log(
                'info',
                'System clock is okay { server: %d, local: %d }',
                [server_time, system_time],
              )
            }
          }
    ...
    this.intervals.retarget = setInterval(() => {
      for (let connection_id in this.connections) {
        ...
        if (miner.varDiff.enabled && miner.retarget()) {
          logger.log(
            'info',
            'Difficulty change { old: %d, new: %d } for %s@%s',
            [
              miner.difficulty.last,
              miner.difficulty.now,
              miner.workerName,
              miner.ip,
            ],
          )
  ...
  startServer() {
    return new Promise((resolve, reject) => {
      this.server = createServer((socket) => {
        ...
        socket
          .on('data', (data) => {
            ...
            if (Buffer.byteLength(buffer, 'utf8') > 10 * 10240) {
              buffer = null
              logger.log('warn', 'Socket flooding from %s', [
              ...
              for (const message of messages) {
                ...
                try {
                  ...
                  if (!json.id || !json.method || !json.params) {
                    logger.log('warn', 'Invalid stratum call from %s', [
                      socket.remoteAddress,
                    ])
                    return
                  }
                } catch (error) {
                  logger.log('warn', 'Malformed stratum call from %s', [
                    socket.remoteAddress,
                  ])
                  socket.destroy()
                  break
                ...
                try {
                  ...
                  logger.log('error', 'Error handling stratum call from %s', [
                    socket.remoteAddress,
                  ])
          ...
          .on('error', (error) => {
            if (error.code !== 'ECONNRESET') {
              logger.log('warn', 'Socket error from %s - %j', [
                socket.remoteAddress,
                error,
              ])
            }
          ...
          .on('close', () => {
            logger.log('warn', 'Socket closed from %s@%s', [
              socket.workerName,
              socket.remoteAddress,
            ])
          })
        ...
        .listen(
          this.config.server.bindPort,
          this.config.server.bindIP,
          (error, result) => {
            if (error) {
              logger.log('error', 'Could not start pool server on port %d', [
                this.config.server.bindPort,
                error,
              ])
              return reject()
            }
            logger.log('info', 'Started server on port %d', [
              this.config.server.bindPort,
            ])
            resolve()
          },
        ...
        .on('error', (error) => {
          if (error.code == 'EADDRINUSE') {
            logger.log('warn', 'Cannot bind on %s:%s - address in use', [
              this.config.server.bindIP,
              this.config.server.bindPort,
            ])
            reject()
  ...
  handleStratum(json, socket) {
    ...
    switch (method) {
      case 'login':
        ...
        let { enabled, startDiff, minDiff, maxDiff, fixedDiffSeparator } =
          this.config.varDiff
        ...
        if (login_parts.length > 1) {
          ...
          if (/^\d+$/.test(login_diff)) {
            ...
            enabled = false
            difficulty = Math.max(
              Math.min(parseInt(login_diff), maxDiff),
              minDiff,
            )
          }
        }
        ...
        logger.log(
          'info',
          'Worker connected { difficulty: %d, fixed: %s } %s@%s',
          [difficulty, fixed, workerName, ip],
        )
      ...
      case 'submit':
        ...
        this.processShare(job, block, nonce, hash)
          .then((result) => {
            logger.log(
              'info',
              'Accepted share { difficulty: %d, actual: %d } from %s@%s',
              [job.difficulty, result.diff, miner.workerName, miner.ip],
            )
            reply(null, { status: 'OK' })
            if (result.hash) {
              logger.log(
                'success',
                'Block found { hash: %s, height: %d } by %s@%s',
                [result.hash, job.height, miner.workerName, miner.ip],
              )
          ...
          .catch((error) => {
            logger.log(
              'info',
              'Rejected share { difficulty: %d, actual: %d } from %s@%s',
              [job.difficulty, error.diff, miner.workerName, miner.ip],
            )
            logger.log('error', '%s { height: %d } from worker %s@%s', [
              error.message,
              job.height,
              miner.workerName,
              miner.ip,
  ...
  processShare(job, block, nonce, hash) {
    return new Promise((resolve, reject) => {
      ...
      if (hash_diff.geq(block.difficulty)) {
        logger.log(
          'info',
          'Block candidate { height: %d, diff: %d, target: %d, nonce: %s }',
          [job.height, hash_diff, block.difficulty, nonce],
        )
        let block_blob = ''
        try {
          ...
          block_blob = this.core_bridge.construct_block_blob(
            template.toString('hex'),
            Buffer.from(nonce, 'hex').readUInt32LE(0),
          )
        } catch (error) {
          return reject({
            message: 'Error constructing block blob',
            diff: hash_diff,
          })
        }
        this.submitBlock(block_blob).then((data) => {
          if (data.hasOwnProperty('error')) {
            return reject({
              message: 'Error submitting block',
              diff: hash_diff,
            })
          }
          let block_fast_hash =
        ...
        return reject({
          message: 'Rejected low difficulty share',
          diff: hash_diff,
        })
      } else {
  ...
  getBlock(force = false) {
    ...
    const uniform =
      this.config.mining.uniform || Object.keys(this.connections).length > 128
    const reserve_size = uniform ? 8 : 1
    return new Promise((resolve, reject) => {
      this.sendRPC('get_block_template', { wallet_address, reserve_size }).then(
        (data) => {
          if (data.hasOwnProperty('error')) {
            logger.log('error', 'Error polling get_block_template %j', [
              data.error.message,
            ])
          ...
          if (
            this.blocks.current == null ||
            this.blocks.current.height < block.height ||
            force
          ) {
            const address_abbr =
              wallet_address.substring(0, 5) +
              '...' +
              wallet_address.substring(wallet_address.length - 5)
            logger.log(
              'info',
              'New block to mine { address: %s, height: %d, difficulty: %d, uniform: %s }',
              [address_abbr, block.height, block.difficulty, uniform],
            )
  ...
  stop() {
    return new Promise((resolve, reject) => {
      ...
      for (let connection_id in this.connections) {
        const miner = this.connections[connection_id]
        logger.log('warn', 'Closing connection %s@%s', [
          miner.workerName,
          miner.ip,
        ])
        miner.socket.destroy()
