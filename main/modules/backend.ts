...
export class Backend {
      ...
      case 'open_explorer':
        ...
        if (params.type == 'tx') {
          require('electron').shell.openExternal(
            `${explorer_url}/tx/${params.id}`,
          )
        } else if (params.type == 'block') {
          require('electron').shell.openExternal(
            `${explorer_url}/block/${params.id}`,
          )
        }
        break
  ...
  startup() {
    ...
    fs.readFile(this.config_file, 'utf8', (err, data) => {
      ...
      if (!fs.existsSync(this.config_data.app.data_dir)) {
        this.send('show_notification', {
          type: 'negative',
          message: 'Error: data storge path not found',
          timeout: 2000,
        })
        this.send('set_app_data', {
          status: {
      ...
      try {
        ...
        this.send('show_notification', {
          type: 'negative',
          message: 'Error: data storge path not writable',
          timeout: 2000,
        })
        this.send('set_app_data', {
          status: {
  ...
  getTooltipLabel() {
    ...
    if (
      Object.keys(daemon_info).length == 0 ||
      Object.keys(wallet_info).length == 0
    )
      return 'Initializing...'
    ...
    else target_height = daemon_info.height

    let daemon_local_pct = 0
    if (daemon_type !== 'remote') {
      let d_pct = parseFloat(
        ((100 * daemon_info.height_without_bootstrap) / target_height).toFixed(
          1,
        ),
      )
      if (
      ...
      else daemon_local_pct = d_pct
    }

    let daemon_pct = 0
    if (daemon_type === 'local') daemon_pct = daemon_local_pct

    let wallet_pct = 0
    let w_pct = parseFloat(
      ((100 * wallet_info.height) / target_height).toFixed(1),
    )
    if (w_pct === 100.0 && wallet_info.height < target_height) wallet_pct = 99.9
    else wallet_pct = w_pct
    ...
    if (daemon_type === 'local') {
      if (
        ...
        wallet_info.height < target_height - 1 &&
        wallet_info.height != 0
      ) {
        status += 'Scanning...'
      } else {
        status += 'Ready'
