...
export class Daemon {
  ...
  start(options) {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const args = [
        ...
        '--rpc-bind-ip',
        options.daemon.rpc_bind_ip,
        '--rpc-bind-port',
        options.daemon.rpc_bind_port,
        '--zmq-rpc-bind-ip',
        options.daemon.zmq_rpc_bind_ip,
        '--zmq-rpc-bind-port',
        options.daemon.zmq_rpc_bind_port,
        '--out-peers',
        options.daemon.out_peers,
        '--in-peers',
        options.daemon.in_peers,
        '--limit-rate-up',
        options.daemon.limit_rate_up,
        '--limit-rate-down',
        options.daemon.limit_rate_down,
        '--log-level',
        options.daemon.log_level,
        '--log-file-level',
        options.daemon.log_level,
      ]
      ...
      let intrvl = setInterval(() => {
        this.sendRPC('get_info').then((data) => {
          if (!data.hasOwnProperty('error')) {
            ...
            if (data.error.cause && data.error.cause.code === 'ECONNREFUSED') {
              // Ignore
            } else {
              clearInterval(intrvl)
  ...
  banPeer(host, seconds = 3600) {
    ...
    this.sendRPC('set_bans', params).then((data) => {
      ...
      this.sendGateway('show_notification', {
        message: 'Banned ' + host + ' until ' + end_time,
        timeout: 2000,
      })
  ...
  timestampToHeight(timestamp, pivot = null, recursion_limit = null) {
    return new Promise((resolve, reject) => {
      ...
      this.getRPC('block_header_by_height', { height: estimated_height }).then(
        (data) => {
          if (data.hasOwnProperty('error') || !data.hasOwnProperty('result')) {
  ...
  sendRPC(method, params = {}, uri: string | false = false) {
    let id = this.id++
    let options = {
   ...
   */
  getRPC(parameter, args = undefined) {
