...
export class WalletRPC {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      crypto.randomBytes(64 + 64 + 32, (err, buffer) => {
        ...
        this.walletRPCProcess.stdout.on('data', (data) => {
          ...
          if (
            height &&
            height > this.wallet_info.height &&
  ...
  restoreWallet(
    ...
    if (refresh_type == 'date') {
      ...
      this.backend.daemon.timestampToHeight(timestamp).then((height) => {
        ...
        else this.restoreWallet(filename, password, seed, 'height', height)
      })
      return
    }
    ...
    switch (seed_words.length) {
      ...
      default:
        this.sendGateway('set_wallet_error', {
          status: { code: -1, message: 'Invalid seed word length' },
        })
        return
    }
  ...
  importWallet(filename, password, import_path) {
    ...
    if (!fs.existsSync(import_path)) {
      ...
      if (fs.existsSync(destination) || fs.existsSync(destination + '.keys')) {
        this.sendGateway('set_wallet_error', {
          status: { code: -1, message: 'Wallet with name already exists' },
        })
        return
      }
  ...
  transfer(
    ...
    ringsize,
    priority,
    address_book: { name?: string; description?: string; save?: boolean } = {},
  ) {
    crypto.pbkdf2(
      password,
      this.auth[2],
      ...
      (err, password_hash) => {
        ...
        amount = parseFloat(parseFloat(amount).toFixed(9)) * 1e9
      ...
      Promise.all([
        ...
        if (
          ...
          for (
            ...
            this.sendRPC('create_address', { account_index: 0 }).then(
              (data) => {
      ...
      this.sendRPC('get_transfers', {
        ...
        for (let i = 0; i < wallet.transactions.tx_list.length; i++) {
          if (/^0*$/.test(wallet.transactions.tx_list[i].payment_id)) {
            ...
            wallet.transactions.tx_list[i].payment_id =
              wallet.transactions.tx_list[i].payment_id.substring(0, 16)
          }
        }
      ...
      this.sendRPC('get_address_book').then((data) => {
        ...
        if (data.result.entries) {
          ...
          for (i = 0; i < data.result.entries.length; i++) {
            ...
            else wallet.address_list.address_book.push(entry)
          }
        }
  ...
  addAddressBook(
    ...
    let params: { address: string; payment_id?: string; description?: string } =
      {
        address,
      }
  ...
  exportKeyImages(password, filename = null) {
    crypto.pbkdf2(
      ...
      (err, password_hash) => {
        ...
        else filename = path.join(filename, 'key_image_export')
      ...
      (err, password_hash) => {
        ...
        this.sendRPC('import_key_images', { filename }).then((data) => {
          if (data.hasOwnProperty('error') || !data.hasOwnProperty('result')) {
            this.sendGateway('show_notification', {
              type: 'negative',
              message: 'Error importing key images',
              timeout: 2000,
            })
            return
          }
  ...
  listWallets(legacy = false) {
    type WalletDataType = {
      name: string
      address: string | null
      password_protected: string | null
    }

    type WalletDataLegacyType = {
      path: string
      address: string
    }

    let wallets: { list: WalletDataType[]; legacy?: WalletDataLegacyType[] } = {
      list: [],
    }
    ...
    fs.readdirSync(this.wallet_dir).forEach((filename) => {
      ...
      let wallet_data: WalletDataType = {
        name: filename,
        address: null,
      ...
      if (fs.existsSync(path.join(this.wallet_dir, filename + '.meta.json'))) {
        let meta = fs.readFileSync(
          path.join(this.wallet_dir, filename + '.meta.json'),
          'utf8',
        )
        if (meta) {
          const parsed_meta: { address: string; password_protected: string } =
            JSON.parse(meta)
          wallet_data.address = parsed_meta.address
          wallet_data.password_protected = parsed_meta.password_protected
        }
      } else if (
        fs.existsSync(path.join(this.wallet_dir, filename + '.address.txt'))
      ) {
        let address = fs.readFileSync(
          path.join(this.wallet_dir, filename + '.address.txt'),
          'utf8',
        )
        if (address) {
    ...
    if (legacy) {
      ...
      if (os.platform() == 'win32') {
        legacy_paths = [
          'C:\\ProgramData\\RyoGUIWallet',
          'C:\\ProgramData\\RyoLITEWallet',
        ]
      } else {
        legacy_paths = [
          path.join(os.homedir(), 'RyoGUIWallet'),
          path.join(os.homedir(), 'RyoLITEWallet'),
        ]
      }
  ...
  sendRPC(method: string, params = {}, timeout = 0) {
    let id = this.id++
    let options: RequiredUriUrl & RequestPromiseOptions = {
